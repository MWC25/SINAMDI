...
export const patientDTO = {
    buildCreatePatientDto(data: any): PatientDTOType {
        if (
            ...
            throw createHttpError(ErrorTypes.BAD_REQUEST,
                'Invalid data: name, cpf, phone, email and birthDate are required.'
            );
        }
        ...
        if (
            ...
            throw createHttpError(ErrorTypes.BAD_REQUEST,
                'Invalid data: Address fields (street, number, neighborhood, city, state, zipCode) are required.'
            );
        }
    ...
    buildUpdatePatientDto(data: any): Partial<PatientDTOType> {
        ...
        if (data.patientStatus)
            dto.patientStatus = data.patientStatus as PatientStatus;
        ...
        if (data.address) {
            ...
            if (data.address.state)
                addressData.state = data.address.state as State;
