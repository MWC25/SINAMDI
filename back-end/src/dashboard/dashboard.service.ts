...
const buildPreviousPeriod = (current: DateRange): DateRange => {
...
const computeMovingAverage7 = (
    serie: Array<{ date: string; count: number }>
): Array<{ date: string; count: number; mm7: number | null }> => {
...
const getOverview = async (params: OverviewParams) => {
    ...
    const [
        ...
        dashboardRepository.getTotalCount(currentPeriod),
        dashboardRepository.getTotalCount(previousPeriod),
        dashboardRepository.getHighRiskCount(currentPeriod),
        dashboardRepository.getHighRiskCount(previousPeriod),
        dashboardRepository.groupByAgeRange(currentPeriod),
        dashboardRepository.groupByState(currentPeriod),
    ...
    const totalVariationPercent =
        N_prev > 0 ? ((N_current - N_prev) * 100) / N_prev : null;

    // Card 2 – % MEDIUM+HIGH global
    const highRiskPercentCurrent =
        N_current > 0 ? (highRiskCurrent * 100) / N_current : 0;
    const highRiskPercentPrev = N_prev > 0 ? (highRiskPrev * 100) / N_prev : 0;
    const highRiskVariationPoints =
        N_prev > 0 ? highRiskPercentCurrent - highRiskPercentPrev : null;
    ...
    const avgRiskVariationAbs =
        N_prev > 0 ? avgRiskCurrent - avgRiskPrev : null;

    const avgRiskVariationPercent =
        N_prev > 0 && avgRiskPrev !== 0
    ...
    const ratePer100kByState = stateDistCurrent
        .map(row => {
            ...
            const ratePer100k =
                pop && pop > 0 ? (row.count * 100_000) / pop : null;
            return {
                state: row.state,
                count: row.count,
                population: pop,
    ...
    const byStateAgg: Record<string, { sumScore: number; total: number }> = {};
    ...
    const avgRiskScoreByState = Object.entries(byStateAgg).map(
        ([state, agg]) => ({
            state,
    ...
    const totalByAgeMap: Record<string, number> = {};
    for (const row of ageDistCurrent) {
        totalByAgeMap[row.ageRange] = row.count;
    }

    const highRiskByAgeMap: Record<string, number> = {};
